...
export default function SearchModal() {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-20">
      ...
      <div className="relative w-full max-w-2xl mx-4 bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-700 rounded-xl shadow-2xl overflow-hidden">
        <div className="p-4 border-b border-gray-200 dark:border-zinc-800">
          <div className="flex items-center gap-3">
            ...
            <input
              ...
              className="flex-1 bg-transparent text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-zinc-500 focus:outline-none text-lg"
            />
            <button onClick={close} className="p-1 text-gray-400 dark:text-zinc-500 hover:text-gray-900 dark:hover:text-white">
          ...
          {!loading && hasResults && (
            <div className="p-2">
              {results.products.length > 0 && (
                <div className="mb-4">
                  <h3 className="px-3 py-2 text-xs font-semibold text-gray-500 dark:text-zinc-500 uppercase">Products</h3>
                  {results.products.map(product => (
                    <button
                      key={product.id}
                      onClick={() => navigateTo('/shop')}
                      className="w-full flex items-center gap-3 px-3 py-2 hover:bg-gray-100 dark:hover:bg-zinc-800 rounded-lg text-left"
                    >
                      <span className="text-xl">📦</span>
                      <div>
                        <div className="text-gray-900 dark:text-white">{product.name}</div>
                        <div className="text-sm text-gray-500 dark:text-zinc-500">${product.price}</div>
                      </div>
                    </button>
                  ))}
                </div>
              ...
              {results.destinations.length > 0 && (
                <div className="mb-4">
                  <h3 className="px-3 py-2 text-xs font-semibold text-gray-500 dark:text-zinc-500 uppercase">Destinations</h3>
                  {results.destinations.map(dest => (
                    ...
                    >
                      ...
                      <div>
                        <div className="text-gray-900 dark:text-white">{dest.name}</div>
                        <div className="text-sm text-gray-500 dark:text-zinc-500">{dest.location}</div>
                      </div>
                    </button>
                  ))}
                </div>
              ...
              {results.posts.length > 0 && (
                <div>
                  <h3 className="px-3 py-2 text-xs font-semibold text-gray-500 dark:text-zinc-500 uppercase">Blog Posts</h3>
